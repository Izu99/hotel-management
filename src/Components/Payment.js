                ...
                if(this.state.exMonth.length === 2){
                    if(this.state.expYear.length === 2){
                      if(this.state.cvv.length === 3){
                            axios.post('http://localhost:4000/payment/paymentAdd',obj)
                                .then(res => {
    ...
    <div>
        ...
        <div className="basic-info">
          <label>Name of card</label>
            <input type ="text" required placeholder = "Please enter correct card name"  value={this.state.cName} onChange = {this.onChangecName}/>
        </div>  
        
        <div className="basic-info">
          <label>Bank Name</label>
          <input type ="text" required placeholder = "Please enter correct bank name"  value={this.state.bName} onChange = {this.onChangebName}/>
        </div>  
        
        <div className="basic-info">
          <label>Credit card number</label>
          <input type ="text" required placeholder = "Please enter correct Credit card number"  value={this.state.cNumber} onChange = {this.onChangecNumber}/>
        </div> 

        <div className="basic-info">
          <label>Exp month</label>
         
          <input type ="text" required placeholder = "Please enter correct Exp month"  value={this.state.exMonth} onChange = {this.onChangeexMonth}/>
        </div>
<div className="basic-info"> 
  <div className='checkout'>
    <label htmlFor="checkout-day">Exp year</label>
    
    <input type ="text" id="checkout-day" min="2020" max="2028" required placeholder = "Please enter correct Exp year"  value={this.state.expYear} onChange = {this.onChangeexpYear}/>
  </div>
</div>

<div className="basic-info">
          <label>CVV</label>
          <input type ="text" required placeholder = "Please enter correct CVV"  value={this.state.cvv} onChange = {this.onChangecvv}/>
        </div> 
